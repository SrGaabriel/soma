...
monomorphizeModule m@AlloyModule{amName = moduleName, amFunctions = funcs} =
    let baseFnMap = toFnMap funcs
        ...
        -- Keep function if:
        -- 1. It's a base function (original, may be polymorphic)
        -- 2. It's a specialized instance (generated by monomorphization)
        -- isKept fn =
        --     afName fn `Set.member` baseNames
        --         || afName fn `Set.member` specializedNames
        
        -- Actually forget about that LOL let's just keep monomorphized functions
        isKept = isMonomorphized
...
applyRewrites baseFnMap rwMap fn@AlloyFunction{afName = callerName, afBlocks, afParams = funParams} =
  ...
  where
    ...
    -- e.g., display([Int]) -> display$Array (polymorphic) which will be monomorphized
    ...
    resolveTraitMethod env methodName args
      ...
      where
        ...
        extractBaseTypeName (TApp (TConstructor (TypeConstructor "Array" _)) _elemTy) = "Array"
        extractBaseTypeName (TConstructor (TypeConstructor name _)) = name
        extractBaseTypeName (TApp _ ty) = extractBaseTypeName ty
