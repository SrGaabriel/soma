...
-- Custom equality that uses Ty.beq for the MonoTy
instance : BEq DictId where
  beq d1 d2 :=
    d1.module == d2.module &&
    d1.className == d2.className &&
    d1.instanceType == d2.instanceType &&
    d1.kind == d2.kind

instance : Hashable DictId where
  hash d := mixHash (hash d.module) (mixHash (hash d.className) (hash d.kind))

def display (d : DictId) : String :=
  match d.kind with
  | .global => s!"Dict${d.className}${d.instanceType}"
...
/-- Kinds of synthetic (compiler-generated) names.
    ...
    Note: refParam stores a string representation of the block name to avoid
    mutual recursion issues with Name. -/
inductive SyntheticKind where
  | liftedLambda
  | closureEnv
  | monomorphized (types : Array MonoTy)
  | instanceMethod (forType : MonoTy)
  | dictParam (className : String) (forType : MonoTy)
  | dictGlobal (className : String) (forType : MonoTy)
...
-- Custom BEq for SyntheticKind
def beq : SyntheticKind → SyntheticKind → Bool
  | .liftedLambda, .liftedLambda => true
  | .closureEnv, .closureEnv => true
  | .monomorphized ts1, .monomorphized ts2 =>
      ts1.size == ts2.size && (ts1.zip ts2).all fun (t1, t2) => t1 == t2
  | .instanceMethod t1, .instanceMethod t2 => t1 == t2
  | .dictParam c1 t1, .dictParam c2 t2 => c1 == c2 && t1 == t2
  | .dictGlobal c1 t1, .dictGlobal c2 t2 => c1 == c2 && t1 == t2
  | .dictStruct c1, .dictStruct c2 => c1 == c2
  | .refParam n1, .refParam n2 => n1 == n2
  | .erasure, .erasure => true
  | .temp, .temp => true
  | _, _ => false

instance : BEq SyntheticKind := ⟨SyntheticKind.beq⟩

def suffix : SyntheticKind → String
  | .liftedLambda => "lambda"
  | .closureEnv => "env"
  | .monomorphized ts => s!"mono${ts.toList.map toString |> String.intercalate "_"}"
...
-- Hashable for SyntheticKind
def hash : SyntheticKind → UInt64
  | .liftedLambda => 0
  | .closureEnv => 1
  | .monomorphized _ => 2  -- Simplified hash
  | .instanceMethod _ => 3
  | .dictParam c _ => mixHash 4 (Hashable.hash c)
  | .dictGlobal c _ => mixHash 5 (Hashable.hash c)
  | .dictStruct c => mixHash 6 (Hashable.hash c)
  | .refParam n => mixHash 7 (Hashable.hash n)
  | .erasure => 8
  | .temp => 9

instance : Hashable SyntheticKind := ⟨SyntheticKind.hash⟩
...
/-- Get instance method type if this is an instance method -/
def instanceMethodType? : Name → Option MonoTy
...
def decomposeInstanceMethod? : Name → Option (Unique × MonoTy)
...
def makeMonomorphized (base : Name) (types : Array MonoTy) : Option Name :=
  makeSynthetic base (.monomorphized types)

/-- Create an instance method name -/
def makeInstanceMethod (base : Name) (forType : MonoTy) : Option Name :=
  makeSynthetic base (.instanceMethod forType)

/-- Create a dictionary parameter name -/
def makeDictParam (base : Name) (className : String) (forType : MonoTy) : Option Name :=
  makeSynthetic base (.dictParam className forType)

/-- Create a dictionary global name -/
def makeDictGlobal (base : Name) (className : String) (forType : MonoTy) : Option Name :=
  makeSynthetic base (.dictGlobal className forType)
...
def mkDictGlobal (moduleName : String) (className : String) (instanceType : MonoTy) : Name :=
  .dict { module := moduleName, className, instanceType, kind := .global }

/-- Create a dictionary struct name directly -/
def mkDictStruct (moduleName : String) (className : String) (instanceType : MonoTy) : Name :=
  .dict { module := moduleName, className, instanceType, kind := .struct }
