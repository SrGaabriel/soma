...
inferExpr env expr = case exprKind expr of
    ...
    VariableReferenceExpr name -> case Map.lookup name env of
...
traverseExpr env expr = do
    ...
    case result of
        ...
        Left UntypedExpression{} -> do
            -- Only suppress UntypedExpression errors
            traverseChildren env expr
...
traverseChildren env expr = do
    ...
    foldM (\s child -> do
        let env' = Map.map (apply s) env
        childSubst <- traverseExpr env' child
        return (composeS childSubst s)
      ) Map.empty children

-- Modified to return Either AnalysisError instead of Maybe
