...
/-- Check if this is an FFI intrinsic operation -/
def isFfiOp : Name → Bool
  | .intrinsic i => i.isFfiOp
  | _ => false

/-- Check if this is an extern function -/
def isExtern : Name → Bool
  | .intrinsic i => i.isExtern
  | _ => false

/-- Get the FFI operation if this is one -/
def ffiOp? : Name → Option FFIOp
  | .intrinsic i => i.ffiOp?
  | _ => none

/-- Get the extern function name if this is one -/
def externName? : Name → Option String
  | .intrinsic i => i.externName?
  | _ => none

/-- Get the Intrinsic if this is an intrinsic name -/
def intrinsic? : Name → Option Intrinsic
  | .intrinsic i => some i
  | _ => none
