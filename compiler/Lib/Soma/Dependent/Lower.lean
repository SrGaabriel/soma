...
private def lowerTypeDecl
  ...
  | .inductive _ name params constructors _ _ =>
    ...
    (some (.algebraic typeName typeVarNames ctors), supply'')
  | .struct _ name params ctorName fields _ =>
    let (typeUnique, supply') := supply.fresh name.value
    let typeName : Soma.Core.QualifiedName := ⟨typeUnique⟩
    let typeVarNames := params.map (·.name.value)
    ...
    (some (.struct typeName typeVarNames ctorQName fieldsWithOptNames), supply'')
  | _ => (none, supply)
